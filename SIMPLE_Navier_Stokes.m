...
Nx = 10;
Ny = 10;
%Pressure Underrelaxation = 0.8, Velocity underrelaxation = 0.5
alpha_u = 0.7;
alpha_v = 0.7;
alpha_p = 0.3;
u_northwall = 5;
...
for i = 3:2:7
    for j = 4:2:8
      ...
      F_u(i-1,j) = 0.5*rho*(vold(i+1,j+1)+vold(i+1,j-1));
      F_u(i,1) = 0.5*rho*dy*(uold(i,2));
      F_u(i+1,2) = 0.5*rho*dx*(vold(i-1,3)+vold(i-1,1));
      F_u(i-1,2) = 0.5*rho*dx*(vold(i+1,3)+vold(i+1,1));
      F_u(10,j) = 0.5*rho*dx*(vold(8,j+1)+vold(8,j-1));
      F_u(9,j-1) = 0.5*rho*dy*(uold(9,j)+uold(9,j-2));
      F_u(9,j+1) = 0.5*rho*dy*(uold(9,j)+uold(9,j+2));
      F_u(9,1) = 0.5*rho*dy*(uold(9,2));
      F_u(10,2) = 0.5*rho*dx*(vold(8,3)+vold(8,1));
    end
...
for j = 3:2:7
    for i = 4:2:8
      ...
      F_v(i+1,j) = 0.5*rho*(vold(i,j)+vold(i+2,j));
      F_v(i,10) = 0.5*rho*dy*(uold(i-1,10)+uold(i+1,10));
      F_v(i-1,9) = 0.5*rho*dx*(vold(i,9)+vold(i-2,9));
      F_v(i+1,9) = 0.5*rho*dx*(vold(i,9)+vold(i+2,9));
      F_v(2,j-1) = 0.5*rho*dy*(uold(1,j-1)+uold(3,j-1));
      F_v(2,j+1) = 0.5*rho*dy*(uold(1,j+1)+uold(3,j+1));
      F_v(1,j) = 0.5*rho*dx*(vold(2,j));
      F_v(2,10) = 0.5*rho*dy*(uold(1,10)+uold(3,10));
      F_v(1,9) = 0.5*rho*dx*(vold(2,9));
